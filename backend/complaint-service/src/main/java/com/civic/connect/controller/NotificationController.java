...
public class NotificationController {
    ...
    private Map<String, Object> mapNotification(Notification n) {
        ...
        map.put("recipientUserId", n.getUser() != null ? n.getUser().getId() : null);
        map.put("recipientRole", (n.getUser() != null && n.getUser().getRole() != null) ? n.getUser().getRole().getName() : "USER");
        map.put("complaintId", n.getComplaintId());
        map.put("title", n.getTitle());
        map.put("message", n.getMessage());
        map.put("type", n.getType());
        map.put("notificationType", n.getType());
        map.put("readStatus", n.getReadStatus() != null ? n.getReadStatus() : false);
        map.put("isRead", n.getReadStatus() != null ? n.getReadStatus() : false);
    ...
    public ResponseEntity<List<Map<String, Object>>> getNotifications(@AuthenticationPrincipal UserPrincipal user) {
        ...
        List<Map<String, Object>> response = list.stream().map(this::mapNotification).collect(Collectors.toList());
        return ResponseEntity.ok(response);
    }

    @GetMapping("/user/{userId}")
    public ResponseEntity<List<Map<String, Object>>> getNotificationsByUserId(@PathVariable("userId") Long userId) {
        List<Notification> list = notificationRepository.findByUserIdOrderByCreatedAtDesc(userId);
        List<Map<String, Object>> response = list.stream().map(this::mapNotification).collect(Collectors.toList());
        return ResponseEntity.ok(response);
    }
