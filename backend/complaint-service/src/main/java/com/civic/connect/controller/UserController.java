...
public class UserController {
    ...
    public ResponseEntity<Map<String, Boolean>> getNotificationPreferences(@AuthenticationPrincipal UserPrincipal currentUser) {
        Long userId = currentUser != null ? currentUser.getId() : 1L;
        return ResponseEntity.ok(userService.getNotificationPreferences(userId));
    }

    @PutMapping("/preferences")
    public ResponseEntity<Map<String, Boolean>> updateNotificationPreferences(@AuthenticationPrincipal UserPrincipal currentUser,
                                                                              @RequestBody Map<String, Boolean> prefs) {
        Long userId = currentUser != null ? currentUser.getId() : 1L;
        return ResponseEntity.ok(userService.updateNotificationPreferences(userId, prefs));
    }
}
